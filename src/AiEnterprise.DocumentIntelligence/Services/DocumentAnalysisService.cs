...
public class DocumentAnalysisService : IDocumentAnalysisService
{
    ...
    public async Task<DocumentAnalysisResult?> GetAnalysisResultAsync(Guid documentId, CancellationToken ct = default)
    {
        ...
        var result = new DocumentAnalysisResult
        {
            ...
            Findings = JsonSerializer.Deserialize<List<DocumentRiskFinding>>(row.Findings) ?? new(),
            KeyClauses = JsonSerializer.Deserialize<List<string>>(row.KeyClauses) ?? new(),
            ComplianceConcerns = JsonSerializer.Deserialize<List<string>>(row.ComplianceConcerns) ?? new(),
            Recommendations = JsonSerializer.Deserialize<List<string>>(row.Recommendations) ?? new(),
